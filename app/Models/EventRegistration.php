...
class EventRegistration extends Model
{
     ...
     */
    public function getHomeEventRegistrationList( Request $request )
    {
        ...
        return $this->transformHomeEventRegistrationContent( $data );
    }

    /**
     * Transform privilege banner information.
     *
     * @param LengthAwarePaginator $homeEventRegistrationList A list of privilege
     *
     * @return LengthAwarePaginator Home banner list for display
     */
    private function transformHomeEventRegistrationContent( LengthAwarePaginator $homeEventRegistrationList )
    {
        foreach( $homeEventRegistrationList as $list ){
            $list->setAttribute( 'title', Utility::getLanguageFields( 'title', $list ) );
            $list->setAttribute( 'image_path', Utility::getImages( Utility::getLanguageFields( 'image_path', $list ) ) );
        }

        return $homeEventRegistrationList;
    }
     ...
     */
    public function getEventRegistrationAllListForAdmin( Request $request )
    {
        $builder = $this->orderBy( 'id', 'desc' );

        $data = Search::search( $builder, 'banner', $request );

        return $this->transformEventRegistrationContent( $data );
    }

    /**
     * Transform banner information.
     *
     * @param LengthAwarePaginator $homeEventRegistrationList A list of banner
     *
     * @return LengthAwarePaginator Home banner list for display
     */
    private function transformEventRegistrationContent( LengthAwarePaginator $homeEventRegistrationList )
    {
        foreach( $homeEventRegistrationList as $list ){
            $list->setAttribute( 'title', Utility::getLanguageFields( 'title', $list ) );
     ...
     *
     * @param EventRegistrationRequest $request EventRegistration request object
     * @param EventRegistration        $banner  EventRegistration model
     *
     * @return array Response information
     */
    public function updateEventRegistrationInformation( EventRegistrationRequest $request, EventRegistration $banner )
    {
     ...
     * @return array Response information
     */
    public function createEventRegistration( EventRegistrationRequest $request )
    {
        ...
        if( $request->file( 'image_path_english' ) ){
            ...
            if( isset( $imageInformation_english['imageInformation']['original'] ) ){
                $image_file_english              = $imageInformation_english['imageInformation']['original'];
                $newEventRegistration['image_path_english'] = $image_file_english;
            }
        }
        ...
        if( $request->file( 'image_path_thai' ) ){
            ...
            if( isset( $imageInformation_thai['imageInformation']['original'] ) ){
                $image_file_thai              = $imageInformation_thai['imageInformation']['original'];
                $newEventRegistration['image_path_thai'] = $image_file_thai;
            }
        }

        $successForEventRegistration = $this->create( $newEventRegistration );

        return [ 'successForEventRegistration' => $successForEventRegistration ];
